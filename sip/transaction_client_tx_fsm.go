...
func (tx *ClientTx) inviteStateAccepted(s fsmInput) fsmInput {
	...
	case client_input_2xx:
		...
		tx.log.Debug("retransimission 2xx detected")
		tx.fsmState, spinfn = tx.inviteStateAccepted, tx.actPassupRetransmission

	case client_input_transport_err:
		tx.log.Warn("client transport error detected. Waiting for retransmission")
...
func (tx *ClientTx) actAckResend() fsmInput {
	...
	if tx.fsmAck != nil {
		...
		tx.log.Error("ACK loop retransimission. Resending after T2")
		select {
		case <-tx.done:
			return FsmInputNone
		case <-time.After(T2):
...
func (tx *ClientTx) passUpRetransmission() {
	...
	case <-time.After(T1):
		tx.log.Debug("skipped response. Retransimission")
	}
}
