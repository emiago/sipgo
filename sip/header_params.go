...
// Set a key to a new value, or adds it if missing.
func (hp *HeaderParams) Set(key string, val string) {
...
// Append a new key-value pair. It doesn't check if it exists already.
// This is a more efficient way to add a missing attribute.
func (hp *HeaderParams) Append(key string, val string) {
	*hp = append(*hp, HeaderKV{K: key, V: val})
}

// Add will add new key-value. If key exists it will be overwritten.
//
// Deprecated: use Set or Append
